...
// Resolve an id for a tool result. Preference:
//   1. the id the caller supplied (sanitized if malformed)
//   2. the next unanswered tool-call id from the preceding assistant message
//   3. fallback generator
function resolveToolResultId(rawId, unanswered, fallback) {
  if (rawId) {
    const sanitized = sanitizeToolId(rawId);
    if (sanitized) {
      const idx = unanswered.indexOf(sanitized);
      if (idx !== -1) unanswered.splice(idx, 1);
      return sanitized;
    }
  }
  if (unanswered.length > 0) {
    return unanswered.shift();
  }
  return fallback();
}
