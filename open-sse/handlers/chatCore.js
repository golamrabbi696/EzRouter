...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onEmptyStream, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, headroomTimeoutMs, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, toolHistoryPruning, translationCache, sourceFormatOverride, providerThinking, toolDisclosure }) {
  ...
  let stream = isCompactRequest ? false : (providerRequiresStreaming ? true : clientRequestedStreaming);
      ...
      if (newCredentials?.accessToken || newCredentials?.copilotToken) {
        ...
        try {
          ...
          if (retryResult.response.ok) {
            ...
            statisticsModel = retryResult.resolvedModel || statisticsModel;
          }
  ...
  if (!providerResponse.ok) {
    ...
    appendRequestLog({ model: statisticsModel, provider, connectionId, status: `FAILED ${statusCode}` }).catch(() => { });
    saveRequestDetail(buildRequestDetail({
      provider, model: statisticsModel, connectionId,
      latency: { ttft: 0, total: Date.now() - requestStartTime },
      tokens: { prompt_tokens: 0, completion_tokens: 0 },
      request: extractRequestConfig(body, stream),
    ...
    const errMsg = formatProviderError(new Error(message), provider, statisticsModel, statusCode);
    if (log?.errorLine) {
  ...
  // Streaming response
  // Streaming response
  const { onStreamComplete, onStreamAborted, streamDetailId } = buildOnStreamComplete({ ...sharedCtx });
  return handleStreamingResponse({ ...sharedCtx, providerResponse, sourceFormat, targetFormat: providerResponseFormat, userAgent, reqLogger, toolNameMap, customToolNames, streamController, onStreamComplete, onStreamAborted, streamDetailId, credentials });
}
