...
Headroom is optional and runs separately. EzRouter calls Headroom's local `/v1/compress` endpoint, then keeps normal routing, fallback, auth, and usage tracking:

```
Client → EzRouter → Headroom /v1/compress → EzRouter → provider
```
...
If Headroom is down or returns an error, EzRouter fails open and sends the original request.
...
- Your CLI tool sends OpenAI format → EzRouter translates → Provider receives native format
...
> EzRouter itself **never charges** you anything. You only pay providers directly (if using paid services).
>
...
### 📊 Understanding EzRouter Costs & Billing

**EzRouter Billing Reality:**

✅ **EzRouter software = FREE forever** (open source, never charges)  
...
❌ **EzRouter never sends invoices** or charges your card
...
- **Cheap providers** (GLM, MiniMax): Pay them directly, EzRouter just routes
- **FREE providers** (iFlow, Kiro, Qwen): Genuinely free forever, no hidden charges
- **EzRouter**: Never charges anything, ever
...
The dashboard tracks your token usage and displays **estimated costs** as if you were using paid APIs directly. This is **not actual billing** - it's a reference to show how much you're saving by using free models or existing subscriptions through EzRouter.
...
<details>
<summary><b>💳 Will I be charged by EzRouter?</b></summary>

**No.** EzRouter is free, open-source software that runs on your own computer. It never charges you anything.

**You only pay:**

- ✅ **Subscription providers** (Claude Code $20/mo, Codex $20-200/mo) → Pay them directly on their websites
- ✅ **Cheap providers** (GLM, MiniMax) → Pay them directly, EzRouter just routes your requests
- ❌ **EzRouter itself** → **Never charges anything, ever**

EzRouter is a local proxy/router. It doesn't have your credit card, can't send invoices, and has no billing system. It's completely free software.
...
EzRouter just routes your requests to them - there's no "catch" or future billing from EzRouter itself. They're truly free services, and EzRouter makes them easy to use with fallback support.
...
3. **Use subscription providers last:**
   ...
   - EzRouter helps maximize their value through quota tracking
...
EzRouter's smart fallback prevents surprise charges:
...
**You're in control:** Set spending limits per provider in dashboard, and EzRouter respects them.
...
**Pro Tip:** Use Opus for complex tasks, Sonnet for speed. EzRouter tracks quota per model!
...
> **Note:** OpenClaw only works with a local instance of EzRouter. Use `127.0.0.1` instead of `localhost` to avoid IPv6 resolution issues.
...
export DATA_DIR="/var/lib/ezrouter"
export PORT="20128"
...
# Or use PM2
npm install -g pm2
pm2 start npm --name ezrouter -- start
pm2 save
pm2 startup
```
...
> **Note:** EzRouter does not publish prebuilt Docker images. Build from source below.
...
git clone https://github.com/golamrabbi696/EzRouter.git
cd EzRouter
docker build -t ezrouter .
docker run -d --name ezrouter -p 20128:20128 \
  -v "$HOME/.ezrouter:/app/data" -e DATA_DIR=/app/data ezrouter
```
...
```bash
docker logs -f ezrouter
docker restart ezrouter
docker stop ezrouter && docker rm ezrouter
```

**Data persistence:** `$HOME/.ezrouter/db/data.sqlite` on host ↔ `/app/data/db/data.sqlite` in container.
