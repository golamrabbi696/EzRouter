...
### 📺完整设置指南 - EzRouter + Claude Code 免费
...
- 您的 CLI 工具发送 OpenAI 格式 → EzRouter 转换 → 提供商接收原生格式
...
> EzRouter 本身**从不向您收费**。您只需直接向提供商付款（如果使用付费服务）。
> 
...
- **廉价提供商**（GLM, MiniMax）：直接向他们付款，EzRouter 只是路由
- **免费**（iFlow, Kiro, Qwen）：真正永远免费，没有隐藏费用
- **EzRouter**：从不收取任何费用，永远
...
仪表板追踪您的 Token 使用情况，并显示**估算成本**，就像您直接使用付费 API 一样。这**不是实际计费** - 它是一个参考，显示您通过 EzRouter 使用免费模型或现有订阅节省了多少。
...
<details>
<summary><b>💳 EzRouter 会向我收费吗？</b></summary>

**不会。** 9 是免费的开源软件，在您自己的计算机上运行。它从不向您收费。

**您只需支付：**
- ✅ **订阅提供商**（Claude Code $20/月, Codex $20-200/月）→ 在他们的网站上直接向他们付款
- ✅ **廉价提供商**（GLM, MiniMax）→ 直接向他们付款，EzRouter 只是路由您的请求
- ❌ **EzRouter 本身** → **从不收取任何费用，永远**

EzRouter 是本地代理/路由器。它没有您的信用卡，不能发送发票，也没有计费系统。完全免费的软件。
...
Router 只是将您的请求路由到它们 - 没有“陷阱”或未来计费。它们是真正的免费服务，EzRouter 使它们易于使用并支持回退。

**注意：** 一些订阅提供商（Antigravity, GitHub Copilot）可能有免费预览期，后来可能变成付费，但这会由这些提供商明确宣布，而不是 EzRouter。
...
**您在控制中：** 在仪表板中设置每个提供商的支出限制，EzRouter 会遵守它们。
...
**专业提示：** 使用 Opus 处理复杂任务，Sonnet 追求速度。EzRouter 追踪每个模型的配额！
...
> **注意：** OpenClaw 仅适用于本地 EzRouter。使用 `127.0.0.1` 而不是 `localhost` 以避免 IPv6 解析问题。
...
- 当上游返回 `text/event-stream` 时，本地 EzRouter 运行时包含 SSE→JSON 回退用于非流式调用。
...
详细指南请参阅 [Pull Requests](https://github.com/golamrabbi696/EzRouter/pulls)。

---

## 🔀 分支

**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — EzRouter 的全功能 TypeScript 分支。添加了 36+ 提供商、4 层自动回退、多模态 API（图像、嵌入、音频、TTS）、熔断器、语义缓存、LLM 评估和精美的仪表板。8+ 单元测试。通过 npm 和 Docker 可用。
