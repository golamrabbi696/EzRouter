<div align="center">
  <img src="../images/9router.png?1" alt="Панель управления EzRouter" width="800"/>
  
  # EzRouter - Free AI Router
  ...
  **Бесплатный AI-провайдер для OpenClaw.**
  ...
  </p>  [![GitHub stars](https://img.shields.io/github/stars/golamrabbi696/EzRouter?style=flat)](https://github.com/golamrabbi696/EzRouter/stargazers)
...
### 📺 Полное руководство по настройке - EzRouter + Claude Code БЕСПЛАТНО
  
[![Настройка EzRouter + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Полное пошаговое руководство:**
- ✅ Установка и настройка EzRouter
...
EzRouter бесшовно работает со всеми основными AI-инструментами для кодинга:
...
- Ваш CLI-инструмент отправляет формат OpenAI → EzRouter транслирует → Провайдер получает родной формат
...
> Сам EzRouter **никогда ничего не взимает** с вас. Вы платите напрямую провайдерам (если используете платные сервисы).
> 
...
### 📊 Понимание затрат и оплаты в EzRouter

**Реальность оплаты EzRouter:**

✅ **Софт EzRouter = БЕСПЛАТНО навсегда** (открытый код, никогда не взимает плату)  
...
❌ **EzRouter никогда не выставляет счёт** и не списывает с вашей карты
...
- **Дешёвые провайдеры** (GLM, MiniMax): Платите им напрямую, EzRouter только маршрутизирует
- **БЕСПЛАТНЫЕ провайдеры** (iFlow, Kiro, Qwen): Действительно бесплатны навсегда, без скрытых платежей
- **EzRouter**: Никогда ничего не взимает, никогда
...
Панель отслеживает ваше использование токенов и показывает **оценочные затраты**, как если бы вы напрямую использовали платный API. Это **не реальная оплата** — это справка, показывающая, сколько вы экономите, используя бесплатные модели или существующие подписки через EzRouter.
...
<details>
<summary><b>💳 Взимает ли с меня плату EzRouter?</b></summary>

**Нет.** EzRouter — это бесплатное ПО с открытым кодом, работающее на вашем собственном компьютере. Оно никогда ничего с вас не взимает.

**Вы платите только:**
- ✅ **Провайдерам подписки** (Claude Code $20/мес, Codex $20-200/мес) → Платите им напрямую на их сайте
- ✅ **Дешёвым провайдерам** (GLM, MiniMax) → Платите им напрямую, EzRouter только маршрутизирует ваши запросы
- ❌ **Самому EzRouter** → **Никогда ничего не взимает, никогда**

EzRouter — это локальный прокси/роутер. У него нет вашей кредитной карты, он не может выставлять счета и не имеет платёжной системы. Это полностью бесплатное ПО.
...
EzRouter только маршрутизирует ваши запросы к ним — никаких «ловушек» или будущих платежей. Это действительно бесплатные сервисы, а EzRouter облегчает их использование с поддержкой резервирования.

**Примечание:** Некоторые провайдеры подписки (Antigravity, GitHub Copilot) могут иметь бесплатные пробные периоды, которые позже становятся платными, но об этом чётко уведомляют сами провайдеры, а не EzRouter.
...
3. **Используйте провайдеров подписки в последнюю очередь:**
   ...
   - EzRouter помогает максимизировать их ценность через отслеживание квоты
...
Умный механизм резервирования EzRouter предотвращает неожиданные расходы:
...
**Вы контролируете:** Установите лимиты расходов на каждого провайдера в панели, и EzRouter будет их соблюдать.
...
**Профи-совет:** Используйте Opus для сложных задач, Sonnet для скорости. EzRouter отслеживает квоту для каждой модели!
...
> **Примечание:** OpenClaw работает только с локальным EzRouter. Используйте `127.0.0.1` вместо `localhost`, чтобы избежать проблем с разрешением имён.
...
- Локальный рантайм EzRouter включает резервирование SSE→JSON для непотоковых вызовов, когда upstream возвращает `text/event-stream`.
...
Спасибо всем, кто помогает делать EzRouter лучше!
...
См. [Pull Requests](https://github.com/golamrabbi696/EzRouter/pulls) для подробных инструкций.

---

## 🔀 Форки

**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Полнофункциональный TypeScript-форк EzRouter. Добавляет 36+ провайдеров, авторезервирование на 4 уровнях, мультимодальный API (изображения, embedding, аудио, TTS), circuit breaker, семантическое кеширование, оценку LLM и доработанную панель. 368+ юнит-тестов. Доступен через npm.
